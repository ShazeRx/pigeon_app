...
class ChannelPaginationTest(TestCase):
    ...
    def test_page_should_display_by_query_size_parameter(self):
        # when
        response = self.client.get('/api/channels/?channel_size=12')
        # then
        self.assertEqual(response.status_code, 200)
    ...
    def test_page_should_display_max_13_items(self):
        # when
        response = self.client.get('/api/channels/?channel_size=14')
        # then
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data['results']), 12)
    ...
    def test_response_should_contain_link_to_next_page(self):
        ...
        # then
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['next'], 'http://testserver/api/channels/?page=2')
    ...
    def test_paginator_should_display_second_page(self):
        # when
        response = self.client.get('/api/channels/?page=2')
        # then
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['previous'], 'http://testserver/api/channels/')
        self.assertEqual(len(response.data['results']), 6)

    def test_should_throw_404_when_page_out_of_range(self):
        # when
        response = self.client.get('/api/channels/?page=4')
        # then
